...
def _relative_strength(row: pd.Series) -> float:
    """Reward stocks outperforming the Nifty 50 over the same lookback window.

    rel_strength (stock return minus Nifty return, in pct points) is computed
    upstream in nse_fetcher. Missing data (None) scores neutral (half marks).
    """
    rs = row.get("rel_strength")
    if rs is None or pd.isna(rs):
        return 0.5 * settings.WEIGHTS["relative_strength"]
    frac = _scale(rs, -5, 15)
    return frac * settings.WEIGHTS["relative_strength"]
...
def _fundamental_score(row: pd.Series) -> float | None:
    """Best-effort bonus (roughly -3..+10) layered on top of the technical score.

    Only meaningful for rows where fundamentals were actually fetched (the
    top FUNDAMENTALS_TOP_N candidates) — returns None otherwise so callers
    can leave every other row's score untouched.
    """
    roe = row.get("roe")
    ...
    if pd.isna(roe) and pd.isna(debt_eq) and pd.isna(growth) and pd.isna(pe):
        return None

    bonus = 0.0
    if pd.notna(roe):
        bonus += _scale(roe * 100, 5, settings.FUND_ROE_GOOD) * 4
    if pd.notna(debt_eq):
        # yfinance's debtToEquity is roughly a percentage of equity (45 == 45%).
        bonus += (1 - _scale(debt_eq, settings.FUND_DEBT_EQUITY_GOOD, 150)) * 3
    if pd.notna(growth):
        bonus += _scale(growth * 100, 0, settings.FUND_GROWTH_GOOD) * 3
    if pd.notna(pe) and pe > settings.FUND_PE_CAUTION:
        bonus -= 3

    return round(bonus, 1)


def apply_fundamental_adjustment(df: pd.DataFrame) -> pd.DataFrame:
    """Nudge final_score using fundamentals, for rows where they were fetched.

    Fundamentals are only available for a subset of candidates, so this is a
    best-effort bonus on top of the technical score — never a hard filter.
    Rows without fundamental data are left completely untouched.
    """
    if df.empty or "roe" not in df.columns:
        return df

    df = df.copy()
    bonus = df.apply(_fundamental_score, axis=1)
    has_fundamentals = bonus.notna()
    df["fundamental_bonus"] = bonus.fillna(0.0)

    df.loc[has_fundamentals, "final_score"] = (
        df.loc[has_fundamentals, "final_score"] + df.loc[has_fundamentals, "fundamental_bonus"]
    ).clip(0, 100).round().astype(int)

    banded = df.loc[has_fundamentals, "final_score"].apply(settings.action_for)
    df.loc[has_fundamentals, "action"] = banded.apply(lambda t: t[0])
    df.loc[has_fundamentals, "position_size"] = banded.apply(lambda t: t[1])
